...
{
    NSUInteger page = item/(self.item_H*self.item_Row);
    
    NSUInteger theX = item % self.item_Row + page * self.item_Row;
    NSUInteger theY = item / self.item_Row - page * self.item_H;
    if (x != NULL) {
        *x = theX;
    }
    if (y != NULL) {
...
- (NSUInteger)originItemAtX:(NSUInteger)x
                          y:(NSUInteger)y
{
    NSUInteger item = x * self.item_H + y;
    return item;
}
